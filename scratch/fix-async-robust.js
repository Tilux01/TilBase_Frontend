...
files.forEach(file => {
  ...
  // 1. Temporarily remove await
  let tempCode = code.replace(/await showModal/g, "showModal");
  
  try {
    const ast = parser.parse(tempCode, {
      sourceType: "module",
      plugins: ["jsx"]
    });
    ...
    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee.name === 'showModal') {
    ...
    if (changed) {
      ...
      let output = generator(ast, { retainLines: true }, tempCode).code;
      // Put await back
      output = output.replace(/showModal\(/g, "await showModal(");
      // But avoid duplicating it if it was already there somewhere else?
      // Our tempCode had NO await showModal, so putting it back for all showModal calls is safe.
      // EXCEPT the context destruction: `const { showModal } = useGlobalModal();`
      output = output.replace(/const \{ await showModal /g, "const { showModal ");
