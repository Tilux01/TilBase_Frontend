    ...
    return (
        <DashboardLayout>
            ...
            <div className="flex flex-col xl:flex-row gap-6 w-full">
                {}
                <div className="flex-1 bg-surface-container-lowest p-8 rounded-xl shadow-sm border border-outline-variant/10 relative overflow-hidden group">
                    ...
                    <div className="w-full aspect-[2/1] bg-surface-container/50 rounded-lg border border-outline-variant/20 relative overflow-hidden bg-[url('https://upload.wikimedia.org/wikipedia/commons/8/80/World_map_-_low_resolution.svg')] bg-no-repeat bg-center bg-cover opacity-80">
                        {regions.map((region) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-4 h-4 rounded-full border-2 shadow-[0_0_15px_rgba(0,0,0,0.5)] ${isActive ? 'bg-primary border-white animate-hologram' : 'bg-surface border-outline-variant'}`}></div>
                                    <span className={`mt-2 text-[10px] font-bold px-2 py-0.5 rounded border backdrop-blur-sm ${isActive ? 'bg-primary/20 text-primary border-primary/50' : 'bg-surface-container/80 text-on-surface-variant border-outline-variant/30'}`}>
                ...
                <div className="w-full xl:w-[400px] flex flex-col gap-6">
                    <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm border border-outline-variant/10">
                        ...
                        <div className="space-y-3">
                            {regions.map(region => (
                                <div key={region.id} className={`flex items-center justify-between p-3 rounded-lg border transition-all ${activeRegions.includes(region.id) ? 'border-primary bg-primary/5' : 'border-outline-variant/20 bg-surface'} ${!region.available ? 'opacity-60' : ''}`}>
