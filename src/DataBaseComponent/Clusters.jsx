    ...
    <div style={{ width: "100%" }}>
      <DashboardLayout>
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <div className="w-full bg-surface-container-low p-5 rounded-xl border border-outline-variant/20 shadow-sm flex flex-col gap-3">
...

          {
            filteredClusters?.slice(0, 15).map((output, index) => {
              return (
                <div key={index} className="w-full bg-surface-container-lowest p-5 rounded-xl border border-outline-variant/20 shadow-sm hover:shadow-md transition-all group flex flex-col lg:flex-row lg:items-center gap-6 relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 min-w-[240px] relative z-10">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1.5">
                        <span style={{ textTransform: "capitalize" }} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide bg-surface-container text-on-primary-fixed-variant">
                  ...
                  {/* Right: Actions */}
                  <div className="flex flex-col lg:items-end gap-3 min-w-[220px] relative z-10 border-t lg:border-t-0 lg:border-l border-outline-variant/10 pt-4 lg:pt-0 lg:pl-6">
                    <div className="bg-surface-container-high/40 px-3 py-1.5 rounded-lg flex items-center justify-between border border-white/40 shadow-inner w-full lg:w-auto">
                      ...
                      <CopyButton textToCopy={output?.Cluster_Password} className="text-primary hover:bg-primary/10 p-1 rounded transition-colors flex items-center justify-center shrink-0" />
                    </div>

                    <div className="flex gap-2 w-full lg:w-auto">
                      <button 
                        onClick={() => navigate(`/cluster/${output?.id}`, { state: { cluster: output } })}
                        className="bg-surface-container-high text-on-primary-fixed-variant px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-surface-container-highest transition-all flex-1 text-center"
                      >
                        View
                      </button>
          ...
          <div className="w-full bg-surface-container-high/40 p-6 rounded-xl flex flex-col md:flex-row items-center justify-between border border-dashed border-outline-variant/40 hover:border-primary/50 transition-all group cursor-pointer shadow-inner mt-2">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-xl bg-surface-container-lowest flex items-center justify-center text-on-surface-variant group-hover:bg-primary/10 group-hover:text-primary transition-colors shadow-sm shrink-0">
